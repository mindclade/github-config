...
def invoke(*arguments, root=ROOT, environment=None):
    ...
    return subprocess.run(
        command + ["--root", str(root), *arguments], cwd=cwd, env=env,
...
class CompilerDeterminismTest(unittest.TestCase):
    ...
    def test_output_is_stable_across_runs_paths_timezone_locale_and_yaml_order(self):
        with tempfile.TemporaryDirectory() as first_temp, tempfile.TemporaryDirectory() as second_temp:
            ...
            repeated = self.compile(ROOT, first / "catalog-b.json", {"TZ": "America/Detroit", "LC_ALL": "C"})
            relocated = self.compile(second, first / "catalog-c.json", {"TZ": "Pacific/Honolulu", "LC_ALL": "C"})
    ...
    def test_tofu_variable_file_wraps_the_identical_catalog(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = invoke(
                "compile", "--output", str(catalog_path),
                "--tofu-var-file", str(variables_path),
            )
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(json.loads(variables_path.read_text())["catalog"], json.loads(catalog_path.read_text()))
