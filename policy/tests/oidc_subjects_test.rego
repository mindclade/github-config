...
canonical_provider_audience := "https://iam.googleapis.com/projects/123456789/locations/global/workloadIdentityPools/github-ci-evidence/providers/verifier"

canonical_provider_subject := {
	...
	"require_immutable_workflow_ref": true,
	"workload_identity_provider_ref": "verifier",
	"service_account_ref": "ci-evidence-verifier",
}

canonical_provider_token := object.union(valid_token, {
	"id": "ci-evidence-verifier-token",
	"audience": canonical_provider_audience,
	"repo": "mindclade@101/infrastructure-live@202",
	"repository": "mindclade/infrastructure-live",
	"workflow_ref": "mindclade/infrastructure-live/.github/workflows/disaster-recovery.yml@refs/heads/main",
	"workload_identity_provider_ref": "verifier",
	"service_account_ref": "ci-evidence-verifier",
})

canonical_provider_attestation := object.union(valid_attestation, {
	"subject_id": "infrastructure-ci-evidence-verifier",
	"repository": "infrastructure-live",
	"immutable_repo": "mindclade@101/infrastructure-live@202",
	"workflow": ".github/workflows/disaster-recovery.yml",
	"workflow_ref": "mindclade/infrastructure-live/.github/workflows/disaster-recovery.yml@refs/heads/main",
	"audience": canonical_provider_audience,
	"workload_identity_provider_ref": "verifier",
...
test_canonical_provider_resource_audience_allowed_only_for_ci_verifier if {
	qualification := {
		"attestation": canonical_provider_attestation,
		"evidence_digest": sprintf("sha256:%s", [crypto.sha256(json.marshal(canonical_provider_attestation))]),
	}
	denials := oidc_subjects.deny with input as {
		"oidc_policy": {"spec": object.union(policy.spec, {
			"audiences": ["canonical-provider-resource"],
			"subjects": [canonical_provider_subject],
		})},
		"tokens": [canonical_provider_token],
		"workflow_qualifications": [qualification],
	}
		with time.now_ns as 1798804800000000000
	count(denials) == 0
}

test_canonical_provider_resource_wrong_pool_denied if {
	wrong_audience := "https://iam.googleapis.com/projects/123456789/locations/global/workloadIdentityPools/other/providers/verifier"
	wrong_token := object.union(canonical_provider_token, {"audience": wrong_audience})
	wrong_attestation := object.union(canonical_provider_attestation, {"audience": wrong_audience})
	qualification := {
		"attestation": wrong_attestation,
		"evidence_digest": sprintf("sha256:%s", [crypto.sha256(json.marshal(wrong_attestation))]),
	}
	denials := oidc_subjects.deny with input as {
		"oidc_policy": {"spec": object.union(policy.spec, {
			"audiences": ["canonical-provider-resource"],
...
test_tampered_attestation_denied if {
	tampered := object.union(valid_qualification, {"attestation": object.union(valid_attestation, {"repository_id": 999})})
	denials := oidc_subjects.deny with input as {
		"oidc_policy": policy,
		"tokens": [valid_token],
